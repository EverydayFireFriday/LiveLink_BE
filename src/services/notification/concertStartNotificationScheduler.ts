 ...
 *
 * 매일 자정(00:00)에 실행되어:
 * 1. 현재~3일 후 공연이 시작되는 콘서트 조회 (performanceDate 기준)
 * 2. 각 콘서트마다 3개의 Job 생성 시도
 *    - 하루 전 알림 (1440분)
 *    - 3시간 전 알림 (180분)
 *    - 1시간 전 알림 (60분)
 * 3. 과거 시간은 자동으로 스킵하여 미래 알림만 등록
 */
...
async function createConcertStartNotificationJobs(): Promise<void> {
  try {
    ...
    // 현재~3일 후 범위 계산 (2일 이내 미등록 알림도 포함)
    const now = new Date();
    ...
    logger.info(
      `📋 Found ${concerts.length} concerts with performances starting in next 3 days`,
    );
    ...
    for (const concert of concerts) {
      ...
      for (const datetime of concert.datetime) {
        ...
        // 현재~3일 범위에 있는지 확인
        if (performanceDate >= now && performanceDate < threeDaysLater) {
