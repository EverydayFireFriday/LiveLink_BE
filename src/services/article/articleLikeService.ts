...
export class ArticleLikeService {
  ...
  async toggleLike(
    ...
    const isCurrentlyLiked = user.likedArticles?.some(id => id.equals(articleObjectId)) || false;

    let newLikesCount = article.likes_count;

    if (isCurrentlyLiked) {
      // --- 좋아요 취소 ---
      const userUpdateResult = await this.userModel.userCollection.updateOne(
        { _id: new ObjectId(userId) },
        { $pull: { likedArticles: articleObjectId } }
      );

      if (userUpdateResult.modifiedCount > 0) {
        await this.articleModel.updateLikesCount(articleId, -1);
  ...
  async checkLikeStatus(
    ...
    const isLiked = user?.likedArticles?.some(id => id.equals(new ObjectId(articleId))) || false;
  ...
  async getUserLikedArticles(
    ...
    const paginatedIds = [...likedArticleIds].reverse().slice((page - 1) * limit, page * limit);
