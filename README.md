...
sequenceDiagram
    ...
    Article->>MongoDB: Save article data
    Article--&gt;>API: Article created
    API--&gt;>Client: 201 Created

    Client->>API: GET /articles
    API->>Article: Get articles list
    Article->>MongoDB: Query articles
    Article--&gt;>API: Articles data
    API--&gt;>Client: 200 OK + Articles
```

### Concert Management Flow
...
sequenceDiagram
    ...
    Client->>API: POST /concerts
    API->>Auth: Verify admin session
    Auth--&gt;>API: Admin authenticated
    API->>Concert: Create concert
    Concert->>MongoDB: Save concert data
    Concert--&gt;>API: Concert created
    API--&gt;>Client: 201 Created

    Client->>API: GET /concerts/search
    API->>Concert: Search concerts
    Concert->>MongoDB: Query with filters
    Concert--&gt;>API: Search results
    API--&gt;>Client: 200 OK + Concerts
```
